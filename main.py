...
prompt_template = ChatPromptTemplate.from_template(
    ...
    {context}
    Question: {question}
    provide a detailed answer with references to the context. If the answer is not in the context, say "I don't know"
    """
)

def format_docs(docs):
    formarted_docs = [doc.page_content for doc in docs]
    return "\n\n".join(formarted_docs)

def retrieve_docs_wirhout_lcel(query):
...
if __name__ == "__main__":
    ...
    # res = retrieve_docs_wirhout_lcel(question)

    res_lcel = create_docs_with_lcel()

    print(res_lcel.invoke({"question": question}))
